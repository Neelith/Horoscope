...
const getKey: RateLimitKeyFunc = (context: any): string => {
	const _authorizationHeader: string = context.req.valid('header')[Const.AuthorizationHeader];
...
export const rateLimiter = async (context: any, next: Next) => {
	...
	if (_userConfiguration === undefined) {
		throw new HTTPException(500, {
			res: context.json(internalServerError(`Not able to process ${Const.AuthorizationHeader} header`), 500),
		});
	}

	if (_userConfiguration === null) {
		throw new HTTPException(400, {
			res: context.json(badRequest(`${Const.AuthorizationHeader} header not valid`), 400),
		});
	}
